# 样式与主题

<cite>
**本文引用的文件**
- [src/app/globals.css](file://src/app/globals.css)
- [src/styles/ui-tokens-glass.css](file://src/styles/ui-tokens-glass.css)
- [src/styles/ui-semantic-glass.css](file://src/styles/ui-semantic-glass.css)
- [postcss.config.mjs](file://postcss.config.mjs)
- [package.json](file://package.json)
- [next.config.ts](file://next.config.ts)
- [src/pages/_document.tsx](file://src/pages/_document.tsx)
- [src/components/ui/primitives/GlassSurface.tsx](file://src/components/ui/primitives/GlassSurface.tsx)
- [src/components/ui/primitives/GlassButton.tsx](file://src/components/ui/primitives/GlassButton.tsx)
- [src/components/LanguageSwitcher.tsx](file://src/components/LanguageSwitcher.tsx)
- [src/lib/style-presets.ts](file://src/lib/style-presets.ts)
- [src/lib/constants.ts](file://src/lib/constants.ts)
- [src/components/selectors/RatioStyleSelectors.tsx](file://src/components/selectors/RatioStyleSelectors.tsx)
- [src/components/ui/config-modals/config-modal-selectors.tsx](file://src/components/ui/config-modals/config-modal-selectors.tsx)
</cite>

## 更新摘要
**变更内容**
- 新增五种艺术风格分类系统（waoo、3d、2d、real、stop_motion）
- 更新样式选择器组件，从grid-based按钮选择迁移到带分类组织的下拉式选择器
- StyleOption接口新增category字段，支持分类优先级处理
- 增强getStyleOptionCategory函数，优化分类识别逻辑
- 新增大量风格选项，涵盖3D动画、2D动画、真人风格、定格动画等
- 增强风格描述系统，提供详细的风格说明和预览功能
- **新增** StyleSelector组件图片预览系统，包括图片加载错误处理、fallback显示方案和响应式预览容器设计

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件面向Waoowaoo的样式与主题系统，系统化梳理Tailwind CSS在本项目的配置与自定义方式，完整呈现Glass UI设计体系的实现（透明度、阴影、渐变、圆角、密度等），并深入解析主题切换机制、暗色模式支持与品牌色彩应用。文档同时覆盖组件样式封装策略、CSS变量体系、响应式设计与动画实现，并给出性能优化、打包策略与浏览器兼容性建议，以及可直接落地的设计系统最佳实践。

**更新** 本版本新增了完整的艺术风格分类系统，支持五种主要风格分类（waoo、3d、2d、real、stop_motion），每个分类包含多个具体的风格选项，提供实时预览和详细描述功能。样式选择器组件已从grid-based按钮选择迁移到带分类组织的下拉式选择器，显著提升了用户体验和可维护性。**新增** StyleSelector组件现在包含完整的图片预览系统，支持图片加载错误处理、fallback显示方案和响应式预览容器设计，为用户提供更直观的风格预览体验。

## 项目结构
样式与主题相关的核心文件分布如下：
- 全局样式入口：src/app/globals.css
- Glass设计令牌：src/styles/ui-tokens-glass.css
- Glass语义样式：src/styles/ui-semantic-glass.css
- PostCSS集成：postcss.config.mjs
- 构建与运行配置：next.config.ts、package.json
- 文档根节点：src/pages/_document.tsx
- 风格常量定义：src/lib/constants.ts
- 样式选择器组件：src/components/selectors/RatioStyleSelectors.tsx
- 配置弹窗选择器：src/components/ui/config-modals/config-modal-selectors.tsx

```mermaid
graph TB
A["全局样式入口<br/>src/app/globals.css"] --> B["Glass设计令牌<br/>ui-tokens-glass.css"]
A --> C["Glass语义样式<br/>ui-semantic-glass.css"]
D["PostCSS配置<br/>postcss.config.mjs"] --> A
E["Next构建配置<br/>next.config.ts"] --> A
F["包管理与依赖<br/>package.json"] --> D
G["文档根节点<br/>pages/_document.tsx"] --> A
H["风格常量定义<br/>constants.ts"] --> I["ART_STYLES数组"]
J["样式选择器组件<br/>RatioStyleSelectors.tsx"] --> K["分类颜色映射"]
L["配置弹窗选择器<br/>config-modal-selectors.tsx"] --> K
I --> J
K --> J
K --> L
```

**图表来源**
- [src/app/globals.css:1-523](file://src/app/globals.css#L1-L523)
- [src/styles/ui-tokens-glass.css:1-96](file://src/styles/ui-tokens-glass.css#L1-L96)
- [src/styles/ui-semantic-glass.css:1-465](file://src/styles/ui-semantic-glass.css#L1-L465)
- [postcss.config.mjs:1-6](file://postcss.config.mjs#L1-L6)
- [next.config.ts:1-24](file://next.config.ts#L1-L24)
- [package.json:112-186](file://package.json#L112-L186)
- [src/pages/_document.tsx:1-14](file://src/pages/_document.tsx#L1-L14)
- [src/lib/constants.ts:137-637](file://src/lib/constants.ts#L137-L637)
- [src/components/selectors/RatioStyleSelectors.tsx:19-26](file://src/components/selectors/RatioStyleSelectors.tsx#L19-L26)
- [src/components/ui/config-modals/config-modal-selectors.tsx:27-34](file://src/components/ui/config-modals/config-modal-selectors.tsx#L27-L34)

**章节来源**
- [src/app/globals.css:1-523](file://src/app/globals.css#L1-L523)
- [src/styles/ui-tokens-glass.css:1-96](file://src/styles/ui-tokens-glass.css#L1-L96)
- [src/styles/ui-semantic-glass.css:1-465](file://src/styles/ui-semantic-glass.css#L1-L465)
- [postcss.config.mjs:1-6](file://postcss.config.mjs#L1-L6)
- [next.config.ts:1-24](file://next.config.ts#L1-L24)
- [package.json:112-186](file://package.json#L112-L186)
- [src/pages/_document.tsx:1-14](file://src/pages/_document.tsx#L1-L14)
- [src/lib/constants.ts:137-637](file://src/lib/constants.ts#L137-L637)
- [src/components/selectors/RatioStyleSelectors.tsx:19-26](file://src/components/selectors/RatioStyleSelectors.tsx#L19-L26)
- [src/components/ui/config-modals/config-modal-selectors.tsx:27-34](file://src/components/ui/config-modals/config-modal-selectors.tsx#L27-L34)

## 核心组件
- Tailwind CSS集成：通过PostCSS插件加载Tailwind，结合Next.js构建管线进行样式生成与打包。
- Glass设计系统：以CSS变量为核心，定义表面、文本、边框、阴影、模糊、圆角、密度与色调系统；通过语义类名抽象组件外观。
- 主题与暗色模式：基于CSS自定义变体与:root/.dark规则，实现明暗两套OKLCH色彩空间下的变量映射。
- 组件封装：以函数式组件包裹语义类名，形成可复用的GlassSurface、GlassButton等基础UI构件。
- 动画与过渡：内置多组关键帧动画，用于流式反馈、悬浮交互与加载状态。
- **新增** 艺术风格分类系统：支持五种主要风格分类（waoo、3d、2d、real、stop_motion），提供实时预览和详细描述功能。
- **新增** 下拉式样式选择器：从grid-based按钮选择迁移到带分类组织的下拉式选择器，支持分类导航和实时预览。
- **新增** 图片预览系统：StyleSelector组件包含完整的图片预览功能，支持图片加载错误处理、fallback显示方案和响应式预览容器设计。

**章节来源**
- [src/app/globals.css:1-523](file://src/app/globals.css#L1-L523)
- [src/styles/ui-tokens-glass.css:1-96](file://src/styles/ui-tokens-glass.css#L1-L96)
- [src/styles/ui-semantic-glass.css:1-465](file://src/styles/ui-semantic-glass.css#L1-L465)
- [postcss.config.mjs:1-6](file://postcss.config.mjs#L1-L6)
- [src/components/ui/primitives/GlassSurface.tsx:1-47](file://src/components/ui/primitives/GlassSurface.tsx#L1-L47)
- [src/components/ui/primitives/GlassButton.tsx:1-67](file://src/components/ui/primitives/GlassButton.tsx#L1-L67)
- [src/lib/constants.ts:137-637](file://src/lib/constants.ts#L137-L637)
- [src/components/selectors/RatioStyleSelectors.tsx:19-26](file://src/components/selectors/RatioStyleSelectors.tsx#L19-L26)

## 架构总览
整体架构围绕"变量令牌—语义样式—组件封装—主题切换—风格分类"的层次展开，确保设计一致性与可维护性。

```mermaid
graph TB
subgraph "设计令牌层"
T1["ui-tokens-glass.css<br/>变量定义"]
end
subgraph "语义样式层"
S1["ui-semantic-glass.css<br/>语义类名"]
end
subgraph "组件封装层"
C1["GlassSurface.tsx"]
C2["GlassButton.tsx"]
C3["StyleSelector.tsx<br/>样式选择器"]
C4["RatioStyleSelectors.tsx<br/>比例选择器"]
end
subgraph "风格分类系统"
SC1["ART_STYLES常量<br/>风格选项"]
SC2["ART_STYLE_CATEGORIES<br/>分类定义"]
SC3["CATEGORY_COLORS<br/>分类颜色映射"]
end
subgraph "主题与环境"
H1["globals.css<br/>:root/.dark/自定义变体"]
P1["postcss.config.mjs<br/>Tailwind集成"]
N1["next.config.ts<br/>构建配置"]
K1["package.json<br/>依赖声明"]
end
T1 --> S1
S1 --> C1
S1 --> C2
S1 --> C3
S1 --> C4
SC1 --> C3
SC2 --> C3
SC3 --> C3
SC3 --> C4
H1 --> S1
P1 --> H1
N1 --> H1
K1 --> P1
```

**图表来源**
- [src/styles/ui-tokens-glass.css:1-96](file://src/styles/ui-tokens-glass.css#L1-L96)
- [src/styles/ui-semantic-glass.css:1-465](file://src/styles/ui-semantic-glass.css#L1-L465)
- [src/components/ui/primitives/GlassSurface.tsx:1-47](file://src/components/ui/primitives/GlassSurface.tsx#L1-L47)
- [src/components/ui/primitives/GlassButton.tsx:1-67](file://src/components/ui/primitives/GlassButton.tsx#L1-L67)
- [src/components/selectors/RatioStyleSelectors.tsx:19-26](file://src/components/selectors/RatioStyleSelectors.tsx#L19-L26)
- [src/lib/constants.ts:137-637](file://src/lib/constants.ts#L137-L637)
- [src/app/globals.css:1-523](file://src/app/globals.css#L1-L523)
- [postcss.config.mjs:1-6](file://postcss.config.mjs#L1-L6)
- [next.config.ts:1-24](file://next.config.ts#L1-L24)
- [package.json:112-186](file://package.json#L112-L186)

## 详细组件分析

### Tailwind CSS配置与自定义
- PostCSS插件：通过[@tailwindcss/postcss](file://postcss.config.mjs#L2)接入Tailwind，确保在Next.js构建链路中自动扫描与生成所需样式。
- 全局样式入口：在[globals.css:1-3](file://src/app/globals.css#L1-L3)中引入Tailwind原语、Glass令牌与语义样式，并定义暗色模式自定义变体。
- 主题与字体：在[globals.css:63-105](file://src/app/globals.css#L63-L105)中通过@theme inline将CSS变量映射到Next.js主题系统，统一字体与颜色命名。
- 构建配置：在[next.config.ts:1-24](file://next.config.ts#L1-L24)中启用国际化插件与图片源配置，保障样式与资源路径一致。

```mermaid
sequenceDiagram
participant Dev as "开发者"
participant PostCSS as "PostCSS"
participant Tailwind as "Tailwind"
participant Next as "Next.js"
participant Browser as "浏览器"
Dev->>PostCSS : 编写样式与变量
PostCSS->>Tailwind : 应用插件与扫描
Tailwind-->>Next : 生成CSS产物
Next-->>Browser : 注入全局样式
Browser-->>Dev : 显示明/暗主题与Glass效果
```

**图表来源**
- [postcss.config.mjs:1-6](file://postcss.config.mjs#L1-L6)
- [src/app/globals.css:1-523](file://src/app/globals.css#L1-L523)
- [next.config.ts:1-24](file://next.config.ts#L1-L24)

**章节来源**
- [postcss.config.mjs:1-6](file://postcss.config.mjs#L1-L6)
- [src/app/globals.css:1-523](file://src/app/globals.css#L1-L523)
- [next.config.ts:1-24](file://next.config.ts#L1-L24)

### Glass UI设计系统
- 令牌层（ui-tokens-glass.css）：集中定义表面背景、文本、边框、阴影、模糊、圆角、密度与色调系统变量，提供默认与低开销预设。
- 语义层（ui-semantic-glass.css）：以语义类名抽象组件外观，如glass-surface、glass-input-base、glass-btn-*、glass-chip-*等，降低对具体变量的耦合。
- 动画与过渡：在[globals.css:137-254](file://src/app/globals.css#L137-L254)中内置多组关键帧动画，用于浮动、渐变、滑入、进度等场景，并尊重用户"减少动态"偏好。

```mermaid
flowchart TD
Start(["开始"]) --> Tokens["读取Glass令牌变量"]
Tokens --> Semantics["应用语义类名"]
Semantics --> Theme{"当前主题？"}
Theme --> |明| LightVars["应用:root变量"]
Theme --> |暗| DarkVars["应用.dark变量"]
LightVars --> Render["渲染组件"]
DarkVars --> Render
Render --> Anim{"是否启用动画？"}
Anim --> |是| Animate["应用关键帧动画"]
Anim --> |否| NoAnimate["降级为静态样式"]
Animate --> End(["结束"])
NoAnimate --> End
```

**图表来源**
- [src/styles/ui-tokens-glass.css:1-96](file://src/styles/ui-tokens-glass.css#L1-L96)
- [src/styles/ui-semantic-glass.css:1-465](file://src/styles/ui-semantic-glass.css#L1-L465)
- [src/app/globals.css:137-254](file://src/app/globals.css#L137-L254)

**章节来源**
- [src/styles/ui-tokens-glass.css:1-96](file://src/styles/ui-tokens-glass.css#L1-L96)
- [src/styles/ui-semantic-glass.css:1-465](file://src/styles/ui-semantic-glass.css#L1-L465)
- [src/app/globals.css:137-254](file://src/app/globals.css#L137-L254)

### 主题切换机制与暗色模式
- 自定义变体：在[globals.css](file://src/app/globals.css#L5)中定义dark自定义变体，使选择器能按容器类名生效。
- 明/暗变量映射：在[:root:7-61](file://src/app/globals.css#L7-L61)与[.dark:481-513](file://src/app/globals.css#L481-L513)中分别映射OKLCH色彩空间的颜色与语义变量，保证视觉一致性。
- 组件内使用：组件通过语义类名间接引用变量，无需感知主题差异，例如按钮、输入框、分段控件等均依赖CSS变量。

```mermaid
sequenceDiagram
participant User as "用户"
participant UI as "界面"
participant DOM as "DOM"
participant CSS as "CSS变量"
User->>UI : 切换明/暗主题
UI->>DOM : 切换根元素类名(.dark)
DOM-->>CSS : 触发变量重映射
CSS-->>DOM : 更新颜色/阴影/模糊
DOM-->>UI : 重新渲染组件外观
```

**图表来源**
- [src/app/globals.css:5-61](file://src/app/globals.css#L5-L61)
- [src/app/globals.css:481-513](file://src/app/globals.css#L481-L513)

**章节来源**
- [src/app/globals.css:5-61](file://src/app/globals.css#L5-L61)
- [src/app/globals.css:481-513](file://src/app/globals.css#L481-L513)

### 组件样式封装策略
- GlassSurface：通过属性控制变体（panel/card/elevated/modal）、密度（compact/default）、交互态与内边距，最终拼接语义类名渲染。
- GlassButton：根据变体（primary/secondary/ghost/danger）与尺寸（sm/md/lg）选择对应语义类，支持左右图标与加载态。
- 通用原则：组件不直接写死颜色或阴影，仅依赖语义类名，便于主题切换与品牌定制。

```mermaid
classDiagram
class GlassSurface {
+variant : "panel"|"card"|"elevated"|"modal"
+density : "compact"|"default"
+interactive : boolean
+padded : boolean
+children
+className
}
class GlassButton {
+variant : "primary"|"secondary"|"ghost"|"danger"
+size : "sm"|"md"|"lg"
+loading : boolean
+iconLeft
+iconRight
+children
}
GlassSurface --> "使用" ui-semantic-glass.css
GlassButton --> "使用" ui-semantic-glass.css
```

**图表来源**
- [src/components/ui/primitives/GlassSurface.tsx:1-47](file://src/components/ui/primitives/GlassSurface.tsx#L1-L47)
- [src/components/ui/primitives/GlassButton.tsx:1-67](file://src/components/ui/primitives/GlassButton.tsx#L1-L67)
- [src/styles/ui-semantic-glass.css:1-465](file://src/styles/ui-semantic-glass.css#L1-L465)

**章节来源**
- [src/components/ui/primitives/GlassSurface.tsx:1-47](file://src/components/ui/primitives/GlassSurface.tsx#L1-L47)
- [src/components/ui/primitives/GlassButton.tsx:1-67](file://src/components/ui/primitives/GlassButton.tsx#L1-L67)
- [src/styles/ui-semantic-glass.css:1-465](file://src/styles/ui-semantic-glass.css#L1-L465)

### 艺术风格分类系统
**更新** Waoowaoo引入了完整的艺术风格分类系统，支持五种主要风格分类：

#### 风格分类定义
- **waoo**：Waoo风格（4个选项）- 保留原有风格
- **3d**：3D风格（9个选项）- 3D动画和渲染风格
- **2d**：2D动画（29个选项）- 传统2D动画风格
- **real**：真人风格（5个选项）- 真实摄影和电影风格
- **stop_motion**：定格动画（5个选项）- 实物拍摄动画风格

#### 分类颜色映射
每种风格分类都有对应的视觉标识：
- waoo：橙色背景（bg-orange-500/20）+ 橙色文字（text-orange-500）
- 3d：蓝色背景（bg-blue-500/20）+ 蓝色文字（text-blue-500）
- 2d：绿色背景（bg-green-500/20）+ 绿色文字（text-green-500）
- real：琥珀色背景（bg-amber-500/20）+ 琥珀色文字（text-amber-500）
- stop_motion：紫色背景（bg-purple-500/20）+ 紫色文字（text-purple-500）

#### 样式选择器组件
**更新** 新的StyleSelector组件支持：
- 分类导航：左侧显示分类列表，右侧显示风格预览
- 实时预览：鼠标悬停时显示风格预览卡片
- 详细描述：显示风格的详细描述信息
- 分类标识：每个风格选项都有对应的分类色块标识
- 分类优先级处理：getStyleOptionCategory函数增强了分类识别逻辑
- **新增** 图片预览系统：右侧预览区域包含完整的图片预览功能

**新增** StyleSelector组件的图片预览系统实现：

```mermaid
flowchart TD
Start(["悬停风格选项"]) --> CheckImg["检查图片是否存在"]
CheckImg --> ImgExists{"图片加载成功？"}
ImgExists --> |是| ShowImg["显示风格图片预览"]
ImgExists --> |否| ShowFallback["显示fallback占位符"]
ShowImg --> CategoryColor["应用分类颜色样式"]
ShowFallback --> CategoryColor
CategoryColor --> ResetError["重置图片加载状态"]
ResetError --> End(["完成"])
```

**图表来源**
- [src/components/selectors/RatioStyleSelectors.tsx:217-241](file://src/components/selectors/RatioStyleSelectors.tsx#L217-L241)
- [src/components/selectors/RatioStyleSelectors.tsx:344-358](file://src/components/selectors/RatioStyleSelectors.tsx#L344-L358)

图片预览系统的关键特性：
- **图片加载错误处理**：使用`onError`回调监听图片加载失败，自动切换到fallback显示
- **响应式预览容器**：预览容器使用`flex-1`和`rounded-xl`实现自适应布局
- **分类颜色映射**：根据预览风格的分类应用相应的背景色和文字色
- **fallback显示方案**：当图片加载失败时，显示包含风格标签和值的占位符
- **状态管理**：使用`useState`管理`imgError`状态，在切换预览风格时重置状态

**章节来源**
- [src/lib/constants.ts:137-637](file://src/lib/constants.ts#L137-L637)
- [src/components/selectors/RatioStyleSelectors.tsx:19-26](file://src/components/selectors/RatioStyleSelectors.tsx#L19-L26)
- [src/components/selectors/RatioStyleSelectors.tsx:236-252](file://src/components/selectors/RatioStyleSelectors.tsx#L236-L252)
- [src/components/selectors/RatioStyleSelectors.tsx:217-241](file://src/components/selectors/RatioStyleSelectors.tsx#L217-L241)
- [src/components/selectors/RatioStyleSelectors.tsx:344-358](file://src/components/selectors/RatioStyleSelectors.tsx#L344-L358)
- [src/components/ui/config-modals/config-modal-selectors.tsx:27-34](file://src/components/ui/config-modals/config-modal-selectors.tsx#L27-L34)

### 品牌色彩与风格预设
- 品牌色：在Glass令牌中定义主色梯度与强调阴影，组件通过语义类名自动获得品牌一致性。
- 风格预设：通过[style-presets.ts:1-25](file://src/lib/style-presets.ts#L1-L25)集中管理风格选项，便于后续扩展不同视觉风格（如恐怖悬疑）。

**章节来源**
- [src/lib/style-presets.ts:1-25](file://src/lib/style-presets.ts#L1-L25)
- [src/styles/ui-tokens-glass.css:69-78](file://src/styles/ui-tokens-glass.css#L69-L78)

### 响应式设计与无障碍
- 响应式：组件与语义类广泛使用Tailwind工具类，配合CSS变量实现跨设备一致体验。
- 无障碍：按钮与输入框遵循焦点可见性与禁用态，滚动条样式在暗色模式下保持高对比度。

**章节来源**
- [src/styles/ui-semantic-glass.css:399-465](file://src/styles/ui-semantic-glass.css#L399-L465)
- [src/components/ui/primitives/GlassButton.tsx:1-67](file://src/components/ui/primitives/GlassButton.tsx#L1-L67)

## 依赖关系分析
- 构建链路：package.json声明tailwind与@tailwindcss/postcss，postcss.config.mjs启用插件，Next.js在构建时注入全局样式。
- 国际化：next.config.ts通过插件增强路由与页面国际化能力，避免样式与文案错位。
- 文档根节点：pages/_document.tsx负责挂载Next脚本与主体，确保样式在SSR/CSR阶段正确渲染。
- **新增** 风格常量：constants.ts提供ART_STYLES和ART_STYLE_CATEGORIES常量，供样式选择器组件使用。
- **新增** 图片预览依赖：StyleSelector组件依赖`/images/style-examples/`目录下的风格图片资源。

```mermaid
graph LR
Pkg["package.json 依赖"] --> Post["postcss.config.mjs 插件"]
Post --> Tw["Tailwind CSS"]
Tw --> Glob["globals.css 入口"]
NextCfg["next.config.ts 配置"] --> Glob
Doc["_document.tsx 根节点"] --> Glob
Const["constants.ts 常量"] --> StyleSel["StyleSelector 组件"]
StyleSel --> Glob
StyleSel --> ImgRes["图片资源 /images/style-examples/"]
```

**图表来源**
- [package.json:164-186](file://package.json#L164-L186)
- [postcss.config.mjs:1-6](file://postcss.config.mjs#L1-L6)
- [src/app/globals.css:1-523](file://src/app/globals.css#L1-L523)
- [next.config.ts:1-24](file://next.config.ts#L1-L24)
- [src/pages/_document.tsx:1-14](file://src/pages/_document.tsx#L1-L14)
- [src/lib/constants.ts:137-637](file://src/lib/constants.ts#L137-L637)
- [src/components/selectors/RatioStyleSelectors.tsx:19-26](file://src/components/selectors/RatioStyleSelectors.tsx#L19-L26)

**章节来源**
- [package.json:164-186](file://package.json#L164-L186)
- [postcss.config.mjs:1-6](file://postcss.config.mjs#L1-L6)
- [src/app/globals.css:1-523](file://src/app/globals.css#L1-L523)
- [next.config.ts:1-24](file://next.config.ts#L1-L24)
- [src/pages/_document.tsx:1-14](file://src/pages/_document.tsx#L1-L14)
- [src/lib/constants.ts:137-637](file://src/lib/constants.ts#L137-L637)
- [src/components/selectors/RatioStyleSelectors.tsx:19-26](file://src/components/selectors/RatioStyleSelectors.tsx#L19-L26)

## 性能考量
- 变量驱动的低耦合：通过CSS变量与语义类名解耦设计与实现，减少重复样式与构建体积。
- 低开销预设：Glass令牌提供subtle预设，适合性能敏感场景，降低模糊与阴影成本。
- 关键帧优化：动画采用关键帧与过渡，避免复杂滤镜；在"减少动态"偏好下自动降级。
- 打包策略：Tailwind按需生成，结合Next.js的Tree Shaking与CSS压缩，减少未使用样式。
- 浏览器兼容：优先使用现代CSS特性（如OKLCH、backdrop-filter），在必要处提供回退方案；通过PostCSS与Tailwind确保兼容性。
- **新增** 样式选择器优化：使用Portal组件避免DOM层级过深，提升渲染性能；下拉式选择器相比grid-based布局减少了不必要的DOM节点。
- **新增** 图片预览性能优化：使用`onError`回调处理图片加载失败，避免阻塞UI渲染；fallback显示方案提供即时反馈。

## 故障排查指南
- 样式未生效
  - 检查globals.css是否被正确导入与编译。
  - 确认postcss.config.mjs已启用@tailwindcss/postcss。
  - 核对next.config.ts的国际化与图片配置是否影响资源路径。
- 暗色模式异常
  - 确保根元素存在.dark类或具备暗色模式触发逻辑。
  - 检查:root与.dark中的变量映射是否完整。
- 动画无效
  - 在系统偏好"减少动态"时，动画会被降级为静态样式，属预期行为。
- 组件外观不一致
  - 确认组件是否使用了正确的语义类名，避免混用自定义样式覆盖变量。
- **新增** 风格选择器问题
  - 检查ART_STYLES常量是否正确导入。
  - 确认分类颜色映射是否完整。
  - 验证样式选择器组件的事件处理逻辑。
  - 检查getStyleOptionCategory函数的分类识别逻辑。
- **新增** 图片预览问题
  - 确认`/images/style-examples/`目录下是否存在对应的风格图片文件。
  - 检查图片URL路径是否正确（格式：`/images/style-examples/${styleValue}.png`）。
  - 验证`imgError`状态管理逻辑是否正常工作。
  - 确认fallback显示方案的样式类名是否正确应用。

**章节来源**
- [src/app/globals.css:1-523](file://src/app/globals.css#L1-L523)
- [postcss.config.mjs:1-6](file://postcss.config.mjs#L1-L6)
- [next.config.ts:1-24](file://next.config.ts#L1-L24)
- [src/lib/constants.ts:137-637](file://src/lib/constants.ts#L137-L637)
- [src/components/selectors/RatioStyleSelectors.tsx:19-26](file://src/components/selectors/RatioStyleSelectors.tsx#L19-L26)

## 结论
Waoowaoo的样式与主题系统以Tailwind为核心，结合Glass设计令牌与语义类名，实现了高内聚、低耦合的组件样式体系。通过CSS变量与暗色模式映射，确保在不同主题下的一致体验；借助语义类名与组件封装，简化了品牌色彩与风格预设的落地。配合合理的性能策略与兼容性处理，该体系既满足设计一致性，又兼顾工程效率与可维护性。

**更新** 新增的艺术风格分类系统进一步增强了设计灵活性，支持五种主要风格分类和大量具体的风格选项，为用户提供更丰富的创作选择。样式选择器组件的从grid-based按钮选择迁移到带分类组织的下拉式选择器，显著提升了用户体验和可维护性。StyleOption接口的category字段和getStyleOptionCategory函数的分类优先级处理，为未来的风格扩展提供了良好的架构基础。**新增** 的StyleSelector组件图片预览系统，包括图片加载错误处理、fallback显示方案和响应式预览容器设计，为用户提供了更直观、更可靠的风格预览体验，进一步完善了整个样式与主题系统。

## 附录
- 实际使用示例（路径）
  - 按钮组件：[src/components/ui/primitives/GlassButton.tsx:1-67](file://src/components/ui/primitives/GlassButton.tsx#L1-L67)
  - 表面容器：[src/components/ui/primitives/GlassSurface.tsx:1-47](file://src/components/ui/primitives/GlassSurface.tsx#L1-L47)
  - 语言切换器（含语义类名）：[src/components/LanguageSwitcher.tsx:91-126](file://src/components/LanguageSwitcher.tsx#L91-L126)
  - 风格预设选项：[src/lib/style-presets.ts:1-25](file://src/lib/style-presets.ts#L1-L25)
  - **新增** 风格常量定义：[src/lib/constants.ts:137-637](file://src/lib/constants.ts#L137-L637)
  - **新增** 样式选择器组件：[src/components/selectors/RatioStyleSelectors.tsx:193-373](file://src/components/selectors/RatioStyleSelectors.tsx#L193-L373)
  - **新增** 配置弹窗选择器：[src/components/ui/config-modals/config-modal-selectors.tsx:129-257](file://src/components/ui/config-modals/config-modal-selectors.tsx#L129-L257)
- 设计令牌与语义样式
  - 令牌：[src/styles/ui-tokens-glass.css:1-96](file://src/styles/ui-tokens-glass.css#L1-L96)
  - 语义：[src/styles/ui-semantic-glass.css:1-465](file://src/styles/ui-semantic-glass.css#L1-L465)
- 构建与配置
  - PostCSS：[postcss.config.mjs:1-6](file://postcss.config.mjs#L1-L6)
  - Next配置：[next.config.ts:1-24](file://next.config.ts#L1-L24)
  - 包依赖：[package.json:112-186](file://package.json#L112-L186)
  - 文档根节点：[src/pages/_document.tsx:1-14](file://src/pages/_document.tsx#L1-14)