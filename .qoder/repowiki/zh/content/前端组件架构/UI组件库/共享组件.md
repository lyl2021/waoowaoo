...
## 更新摘要
**变更内容**
- 更新了ImagePreviewModal的UI优化部分，反映最新的响应式布局优化、按钮定位系统重构、最大宽度扩展至94vw、最大高度调整为90vh、圆角优化为rounded-2xl
- 新增了按钮定位系统的详细说明和绝对定位实现分析
- 更新了组件的样式规范和设计原则
...
**图表来源**
- [ImagePreviewModal.tsx:1-85](file://src/components/ui/ImagePreviewModal.tsx#L1-L85)
...
## 核心组件
- ImagePreviewModal：提供图片预览弹窗，支持 ESC 关闭、遮罩点击关闭、查看原图链接、毛玻璃样式与无障碍文案。**已更新**：采用新的响应式布局设计，最大宽度扩展至94vw，最大高度调整为90vh，按钮定位系统重构为绝对定位，圆角优化为rounded-2xl。
...
- [ImagePreviewModal.tsx:40-83](file://src/components/ui/ImagePreviewModal.tsx#L40-L83)
...
- **重大UI优化**：
  - 响应式布局：最大宽度从 `max-w-7xl` 扩展到 `max-w-[94vw]`，最大高度调整为 `max-h-[90vh]`，提供更好的屏幕适配
  - 按钮定位系统：重构为绝对定位，关闭按钮位于右上角 `top-4 right-4`，原图链接按钮位于 `top-4 right-20`
  - 圆角优化：统一使用 `rounded-2xl` 圆角，提供更柔和的视觉效果
...
ResolveUrls --> Render["渲染遮罩与按钮<br/>采用绝对定位系统"]
Render --> LoadImg["使用 MediaImageWithLoading 加载图片<br/>max-w-[94vw]<br/>max-h-[90vh]<br/>rounded-2xl"]
...
**图表来源**
- [ImagePreviewModal.tsx:14-83](file://src/components/ui/ImagePreviewModal.tsx#L14-L83)
...
- 媒体加载优化：
  ...
  - MediaImageWithLoading 在加载前显示骨架与指示器，加载完成后渐显图片，支持错误回退与自定义类名合并
- 查询与渲染：
...
## 结论
Waoowaoo 的共享组件通过清晰的接口抽象、模块解耦与通用 UI/媒体封装，实现了跨模块的高复用性与一致体验。ImagePreviewModal、GlobalAssetPicker、CharacterCreationModal 等组件在保持低耦合的同时，提供了良好的扩展空间。**最新优化**：ImagePreviewModal 的响应式布局设计显著提升了屏幕适配能力，按钮定位系统重构提供了更好的用户体验，最大宽度和高度的调整确保了在各种设备上的最佳显示效果。建议后续在资产类型扩展、多图轮播、缓存策略与版本化管理方面持续演进，配合完善的测试与质量保障机制，进一步提升稳定性与可维护性。
...
**更新**：ImagePreviewModal 采用了全新的响应式布局设计系统，提供更加灵活和适配性强的视觉体验。

#### 设计原则
- **视口适配**：使用 `max-w-[94vw]` 和 `max-h-[90vh]` 确保在各种屏幕尺寸下的最佳显示效果
- **绝对定位**：按钮采用绝对定位系统，确保在图片上方的正确层级关系和精确位置控制
- **圆角统一**：采用 `rounded-2xl` 圆角，与整体设计语言保持一致
- **阴影系统**：添加 `shadow-2xl` 阴影，增强立体感和层次感

#### 样式特性
- **背景模糊**：使用 `backdrop-blur-sm` 实现背景虚化效果
- **过渡动画**：按钮悬停时的颜色变化提供流畅的交互反馈
- **响应式约束**：通过视口单位确保在移动设备上的良好表现
- **层级管理**：按钮使用 `z-10` 确保在图片上方的正确显示顺序

#### 技术实现
- **CSS 变量**：利用 `--glass-overlay` 变量实现主题化的玻璃效果
- **组件复用**：与全局玻璃拟态样式系统保持一致，便于维护和扩展
- **无障碍支持**：保留完整的无障碍属性和键盘导航支持
- **性能优化**：使用 `object-contain` 确保图片比例正确且性能最优
