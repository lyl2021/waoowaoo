...
## 核心组件
- ImagePreviewModal：提供图片预览弹窗，支持 ESC 关闭、遮罩点击关闭、查看原图链接、毛玻璃样式与无障碍文案。
...
ResolveUrls --> Render["渲染遮罩与关闭按钮"]
Render --> LoadImg["使用 MediaImageWithLoading 加载图片"]
LoadImg --> UserAction{"用户操作"}
UserAction --> |ESC| Close["调用 onClose 并清理样式"]
UserAction --> |点击遮罩| Close
UserAction --> |点击原图链接| OpenOriginal["打开原图链接"]
Close --> End(["退出"])
OpenOriginal --> End
NullReturn --> End
```
...
## 结论
Waoowaoo 的共享组件通过清晰的接口抽象、模块解耦与通用 UI/媒体封装，实现了跨模块的高复用性与一致体验。ImagePreviewModal、GlobalAssetPicker、CharacterCreationModal 等组件在保持低耦合的同时，提供了良好的扩展空间。建议后续在资产类型扩展、多图轮播、缓存策略与版本化管理方面持续演进，配合完善的测试与质量保障机制，进一步提升稳定性与可维护性。
