...
- AI设计角色与场景：ai-design-character、ai-design-location
...
A3["ai-modify-character<br/>POST /api/asset-hub/ai-modify-character"]
A4["ai-modify-location<br/>POST /api/asset-hub/ai-modify-location"]
A5["ai-modify-prop<br/>POST /api/asset-hub/ai-modify-prop"]
A6["generate-image<br/>POST /api/asset-hub/generate-image"]
A7["modify-image<br/>POST /api/asset-hub/modify-image"]
A8["upload-image<br/>POST /api/asset-hub/upload-image"]
A9["select-image<br/>POST /api/asset-hub/select-image"]
A10["undo-image<br/>POST /api/asset-hub/undo-image"]
end
subgraph "通用层"
...
## 结论
资产中心API以“任务化+服务化”为核心设计，既保证了AI生成/修改的异步能力，又通过统一鉴权与错误处理提升了稳定性与可观测性。图像类操作（生成、修改、上传、选择、撤销）形成闭环，配合全局资产共享能力，满足从AI创意到人工微调再到最终确认的完整工作流。
