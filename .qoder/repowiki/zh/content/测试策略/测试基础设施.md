...
- 测试运行器与配置
  ...
  - 核心基线覆盖率配置：[vitest.core-coverage.config.ts](file://vitest.core-coverage.config.ts)
...
## 结论
Waoowaoo 的测试基础设施以 Vitest 为核心，结合 Docker 容器化、Prisma 数据管理与丰富的夹具/断言工具，形成了从环境准备到报告产出的完整闭环。通过分层测试脚本与灵活的覆盖率配置，既能满足日常开发的快速反馈，也能支撑系统级与回归测试的稳定性验证。建议在 CI 中按模块拆分并行执行，持续优化覆盖率与超时策略，并保持测试数据与环境的隔离与可重复性。
