# 小说推广工作流

<cite>
**本文引用的文件**
- [src/lib/workers/index.ts](file://src/lib/workers/index.ts)
- [src/lib/workers/text.worker.ts](file://src/lib/workers/text.worker.ts)
- [src/lib/workers/image.worker.ts](file://src/lib/workers/image.worker.ts)
- [src/lib/workers/video.worker.ts](file://src/lib/workers/video.worker.ts)
- [src/lib/workers/voice.worker.ts](file://src/lib/workers/voice.worker.ts)
- [src/lib/workflow-concurrency.ts](file://src/lib/workflow-concurrency.ts)
- [src/lib/run-runtime/workflow.ts](file://src/lib/run-runtime/workflow.ts)
- [prisma/schema.prisma](file://prisma/schema.prisma)
- [prisma/schema.sqlit.prisma](file://prisma/schema.sqlit.prisma)
- [src/types/project.ts](file://src/types/project.ts)
- [lib/prompts/novel-promotion/agent_storyboard_plan.en.txt](file://lib/prompts/novel-promotion/agent_storyboard_plan.en.txt)
- [lib/prompts/novel-promotion/agent_storyboard_detail.en.txt](file://lib/prompts/novel-promotion/agent_storyboard_detail.en.txt)
- [lib/prompts/novel-promotion/ai_story_expand.en.txt](file://lib/prompts/novel-promotion/ai_story_expand.en.txt)
- [src/lib/model-gateway/index.ts](file://src/lib/model-gateway/index.ts)
- [src/lib/generators/vidu.ts](file://src/lib/generators/vidu.ts)
- [tests/unit/generator-api.test.ts](file://tests/unit/generator-api.test.ts)
- [src/lib/workers/handlers/script-to-storyboard.ts](file://src/lib/workers/handlers/script-to-storyboard.ts)
- [src/lib/workers/text.worker.ts](file://src/lib/workers/text.worker.ts)
- [src/lib/workers/image.worker.ts](file://src/lib/workers/image.worker.ts)
- [src/lib/workers/video.worker.ts](file://src/lib/workers/video.worker.ts)
- [src/lib/workers/voice.worker.ts](file://src/lib/workers/voice.worker.ts)
- [src/app/api/asset-hub/ai-design-prop/route.ts](file://src/app/api/asset-hub/ai-design-prop/route.ts)
- [src/lib/workers/handlers/asset-hub-ai-design.ts](file://src/lib/workers/handlers/asset-hub-ai-design.ts)
- [src/lib/asset-utils/ai-design.ts](file://src/lib/asset-utils/ai-design.ts)
- [src/lib/workers/handlers/modify-description-sync.ts](file://src/lib/workers/handlers/modify-description-sync.ts)
- [src/lib/workers/handlers/shot-ai-prompt-prop.ts](file://src/lib/workers/handlers/shot-ai-prompt-prop.ts)
- [lib/prompts/novel-promotion/prop_create.en.txt](file://lib/prompts/novel-promotion/prop_create.en.txt)
- [lib/prompts/novel-promotion/prop_create.zh.txt](file://lib/prompts/novel-promotion/prop_create.zh.txt)
- [src/components/shared/assets/prop-creation/hooks/usePropCreationSubmit.ts](file://src/components/shared/assets/prop-creation/hooks/usePropCreationSubmit.ts)
- [src/lib/task/types.ts](file://src/lib/task/types.ts)
- [src/lib/llm-observe/route-task.ts](file://src/lib/llm-observe/route-task.ts)
- [tests/unit/worker/asset-hub-ai-design.test.ts](file://tests/unit/worker/asset-hub-ai-design.test.ts)
- [src/app/[locale]/dev/workspace-redesign/page.tsx](file://src/app/[locale]/dev/workspace-redesign/page.tsx)
- [src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx](file://src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx)
- [messages/zh/workspaceRedesign.json](file://messages/zh/workspaceRedesign.json)
- [src/components/ui/SegmentedControl.tsx](file://src/components/ui/SegmentedControl.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/NovelPromotionWorkspace.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/NovelPromotionWorkspace.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/ConfigStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/ConfigStage.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx)
- [src/components/story-input/LongTextDetectionPrompt.tsx](file://src/components/story-input/LongTextDetectionPrompt.tsx)
- [src/lib/episode-marker-detector.ts](file://src/lib/episode-marker-detector.ts)
- [src/lib/workers/handlers/episode-split.ts](file://src/lib/workers/handlers/episode-split.ts)
- [lib/prompts/novel-promotion/episode_split.en.txt](file://lib/prompts/novel-promotion/episode_split.en.txt)
- [lib/prompts/novel-promotion/episode_split.zh.txt](file://lib/prompts/novel-promotion/episode_split.zh.txt)
- [messages/zh/novel-promotion.json](file://messages/zh/novel-promotion.json)
- [messages/zh/smartImport.json](file://messages/zh/smartImport.json)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/SmartImportWizard.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/SmartImportWizard.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/smart-import/hooks/useWizardState.ts](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/smart-import/hooks/useWizardState.ts)
- [src/app/[locale]/workspace/[projectId]/page.tsx](file://src/app/[locale]/workspace/[projectId]/page.tsx)
</cite>

## 更新摘要
**所做更改**
- 更新智能章节建议功能：将触发阈值从1000字符降低到150字符，使更多用户能够受益于AI驱动的章节推荐功能
- 新增智能章节建议功能的详细说明和使用场景
- 更新相关组件和提示文案以反映新的阈值设置
- 补充智能分集流程的完整说明

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [智能章节建议功能](#智能章节建议功能)
7. [AI设计道具功能](#ai设计道具功能)
8. [工作区UI重构](#工作区ui重构)
9. [依赖关系分析](#依赖关系分析)
10. [性能考量](#性能考量)
11. [故障排查指南](#故障排查指南)
12. [结论](#结论)
13. [附录](#附录)

## 简介
本文件面向Waoowaoo的小说推广自动化工作流，系统性阐述从"文本输入"到"视频输出"的完整流水线，覆盖文本分析、分镜生成、图像生成、视频合成与音频处理五大阶段。文档同时解释AI模型选择与调用策略（OpenAI、Gemini等兼容通道）、参数配置与质量控制机制，并提供可操作的使用场景、错误处理与重试策略、性能优化建议及最佳实践。

**更新** 本次更新重点增加了智能章节建议功能的改进说明，将触发阈值从1000字符降低到150字符，使更多用户能够受益于AI驱动的章节推荐功能。

## 项目结构
该工作流以"任务驱动 + 多队列并行"的方式组织，分为文本、图像、视频、语音四类Worker，分别处理不同阶段的任务；项目数据库采用Prisma管理，配套任务类型与并发控制模块支撑整体运行。新的工作区UI重构引入了现代化的界面设计和交互模式。

```mermaid
graph TB
subgraph "前端与入口"
UI["用户界面<br/>项目与工作区"]
API["API路由<br/>novel-promotion接口"]
AH_API["资产中心API<br/>ai-design-prop接口"]
RED设计["工作区重设计<br/>五种布局变体"]
SEG控制["分段控制组件<br/>iOS风格指示器"]
SMART_SPLIT["智能章节建议<br/>阈值150字符"]
end
subgraph "任务与队列"
Q_TEXT["队列: TEXT"]
Q_IMAGE["队列: IMAGE"]
Q_VIDEO["队列: VIDEO"]
Q_VOICE["队列: VOICE"]
Q_AH["队列: ASSET_HUB"]
end
subgraph "Worker执行器"
W_TEXT["文本Worker"]
W_IMAGE["图像Worker"]
W_VIDEO["视频Worker"]
W_VOICE["语音Worker"]
W_AH["资产Hub Worker"]
end
subgraph "数据与模型"
PRISMA["Prisma模型<br/>项目/剧集/分镜/镜头"]
MODELS["模型网关<br/>OpenAI/Gemini等"]
TASK_TYPES["任务类型<br/>含道具设计"]
END
UI --> API
UI --> AH_API
UI --> RED设计
UI --> SEG控制
UI --> SMART_SPLIT
API --> Q_TEXT
API --> Q_IMAGE
API --> Q_VIDEO
API --> Q_VOICE
AH_API --> Q_AH
RED设计 --> Q_TEXT
RED设计 --> Q_IMAGE
RED设计 --> Q_VIDEO
RED设计 --> Q_VOICE
Q_TEXT --> W_TEXT
Q_IMAGE --> W_IMAGE
Q_VIDEO --> W_VIDEO
Q_VOICE --> W_VOICE
Q_AH --> W_AH
W_TEXT --> PRISMA
W_IMAGE --> PRISMA
W_VIDEO --> PRISMA
W_VOICE --> PRISMA
W_AH --> PRISMA
W_TEXT --> MODELS
W_IMAGE --> MODELS
W_VIDEO --> MODELS
W_VOICE --> MODELS
W_AH --> MODELS
TASK_TYPES --> Q_AH
```

**图表来源**
- [src/lib/workers/index.ts](file://src/lib/workers/index.ts)
- [src/lib/workers/text.worker.ts](file://src/lib/workers/text.worker.ts)
- [src/lib/workers/image.worker.ts](file://src/lib/workers/image.worker.ts)
- [src/lib/workers/video.worker.ts](file://src/lib/workers/video.worker.ts)
- [src/lib/workers/voice.worker.ts](file://src/lib/workers/voice.worker.ts)
- [src/lib/workers/handlers/asset-hub-ai-design.ts](file://src/lib/workers/handlers/asset-hub-ai-design.ts)
- [prisma/schema.prisma](file://prisma/schema.prisma)
- [src/lib/task/types.ts](file://src/lib/task/types.ts)
- [src/app/[locale]/dev/workspace-redesign/page.tsx](file://src/app/[locale]/dev/workspace-redesign/page.tsx)
- [src/components/ui/SegmentedControl.tsx](file://src/components/ui/SegmentedControl.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx)

**章节来源**
- [src/lib/workers/index.ts](file://src/lib/workers/index.ts)
- [prisma/schema.prisma](file://prisma/schema.prisma)
- [src/lib/task/types.ts](file://src/lib/task/types.ts)
- [src/app/[locale]/dev/workspace-redesign/page.tsx](file://src/app/[locale]/dev/workspace-redesign/page.tsx)
- [src/components/ui/SegmentedControl.tsx](file://src/components/ui/SegmentedControl.tsx)

## 核心组件
- 任务与并发控制
  - 工作流并发配置：支持独立配置分析、图像、视频三类任务的并发上限，并对非法值进行归一化处理。
  - 任务类型判定：区分AI类任务与非AI任务，便于统一调度与计费。
  - **新增** 资产Hub任务类型：包含角色、场景、道具的AI设计和修改任务。
- 数据模型
  - 小说推广项目模型包含分析/图像/角色/场景/分镜/编辑/视频/语音等模型键，以及画幅、分辨率、TTS速率、工作流模式、风格提示等关键字段。
- Worker与队列
  - 文本Worker：负责故事扩展、剧本转换、分镜计划与细化、角色/场景/道具资产设计与修改、语音分析、剪辑片段构建等。
  - 图像Worker：负责角色/场景/面板图像生成与变体生成、资产修改与图库批量处理。
  - 视频Worker：负责单面板视频生成、首尾帧模式、唇同步、结果持久化与上传。
  - 语音Worker：负责配音生成与语音设计。
  - **新增** 资产Hub Worker：专门处理资产中心的AI设计任务，包括去重检测和进度报告。
- 模型网关与生成器
  - 统一OpenAI兼容通道，按模型键路由至官方或兼容提供方；部分视频生成器支持选项校验与默认值。
- **新增** 工作区UI组件
  - 分段控制组件：iOS风格的滑动胶囊指示器，支持紧凑和填充两种布局模式。
  - 项目布局组件：五种不同的项目展示变体，支持网格、滚动、列表、突出首项和极简列表。
  - 工作区重设计页面：提供布局切换器和交互式演示。
- **新增** 智能章节建议功能
  - 触发阈值：从1000字符降低到150字符，使更多用户能够受益
  - 长文本检测提示：当文本超过阈值时弹出智能分集建议
  - 智能分集流程：支持标记检测和AI自动分集两种模式

**章节来源**
- [src/lib/workflow-concurrency.ts](file://src/lib/workflow-concurrency.ts)
- [src/lib/run-runtime/workflow.ts](file://src/lib/run-runtime/workflow.ts)
- [prisma/schema.prisma](file://prisma/schema.prisma)
- [prisma/schema.sqlit.prisma](file://prisma/schema.sqlit.prisma)
- [src/lib/workers/text.worker.ts](file://src/lib/workers/text.worker.ts)
- [src/lib/workers/image.worker.ts](file://src/lib/workers/image.worker.ts)
- [src/lib/workers/video.worker.ts](file://src/lib/workers/video.worker.ts)
- [src/lib/workers/voice.worker.ts](file://src/lib/workers/voice.worker.ts)
- [src/lib/workers/handlers/asset-hub-ai-design.ts](file://src/lib/workers/handlers/asset-hub-ai-design.ts)
- [src/lib/model-gateway/index.ts](file://src/lib/model-gateway/index.ts)
- [src/lib/generators/vidu.ts](file://src/lib/generators/vidu.ts)
- [src/components/ui/SegmentedControl.tsx](file://src/components/ui/SegmentedControl.tsx)
- [src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx](file://src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx)

## 架构总览
下图展示从文本输入到视频输出的关键交互路径，包括任务提交、Worker执行、模型调用与数据持久化，以及新的工作区UI重构和智能章节建议功能。

```mermaid
sequenceDiagram
participant U as "用户"
participant UI as "工作区UI"
participant API as "API层"
participant AH_API as "资产中心API"
participant Q as "队列"
participant W as "Worker"
participant AH_W as "资产Hub Worker"
participant M as "模型网关/生成器"
participant DB as "数据库"
U->>UI : 输入小说内容
UI->>UI : 检测文本长度(阈值150字符)
UI->>UI : 弹出智能分集建议提示
U->>UI : 选择智能分集或继续创作
UI->>API : 提交小说推广任务
U->>AH_API : 提交道具设计任务
API->>Q : 入队(文本/图像/视频/语音)
AH_API->>Q : 入队(资产Hub任务)
Q-->>W : 取出任务
Q-->>AH_W : 取出资产Hub任务
AH_W->>M : 调用AI模型(道具设计)
W->>M : 调用AI模型(文本/图像/视频/语音)
M-->>AH_W : 返回道具设计结果
M-->>W : 返回结果(文本/图像/视频/音频)
AH_W->>DB : 写入/更新道具资产状态
W->>DB : 写入/更新项目/剧集/分镜/镜头状态
AH_W-->>AH_API : 任务完成事件
W-->>API : 任务完成事件
API-->>UI : 推送进度/结果
AH_API-->>UI : 推送道具设计结果
UI-->>U : 更新界面状态
```

**图表来源**
- [src/lib/workers/text.worker.ts](file://src/lib/workers/text.worker.ts)
- [src/lib/workers/image.worker.ts](file://src/lib/workers/image.worker.ts)
- [src/lib/workers/video.worker.ts](file://src/lib/workers/video.worker.ts)
- [src/lib/workers/voice.worker.ts](file://src/lib/workers/voice.worker.ts)
- [src/lib/workers/handlers/asset-hub-ai-design.ts](file://src/lib/workers/handlers/asset-hub-ai-design.ts)
- [src/lib/model-gateway/index.ts](file://src/lib/model-gateway/index.ts)
- [prisma/schema.prisma](file://prisma/schema.prisma)
- [src/app/[locale]/dev/workspace-redesign/page.tsx](file://src/app/[locale]/dev/workspace-redesign/page.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx)

## 详细组件分析

### 文本分析与分镜生成
- 阶段划分
  - 阶段1：分镜计划（基于剪辑片段与角色/场景/道具库生成初始面板序列）。
  - 阶段2：摄影规则（镜头语言、灯光、色调、氛围等）。
  - 阶段3：表演指导（角色动作、走位、情绪表达）。
  - 细节细化：在保持语义一致的前提下，增强画面细节与动作提示。
- 关键提示模板
  - 分镜计划模板：要求严格的时间顺序、避免遗漏关键情节、使用具体可见动作、保留可用位置锚点等。
  - 细节细化模板：强调动作提示的"可运动化"，保留稳定位置锚点，必要时允许移除或自由放置。
  - 故事扩展模板：将关键词/大纲扩展为适合1-2分钟短剧的完整故事，限定字数范围与格式要求。
- 并发与流式回调
  - 文本Worker对各阶段采用并行执行与流式回调，实时上报进度与推理/输出片段。
- 任务编排
  - ScriptToStoryboard任务会读取选定剪辑片段与资产库，组装提示模板并触发多阶段推理。

```mermaid
flowchart TD
Start(["开始: 收到ScriptToStoryboard任务"]) --> LoadAssets["加载项目资产:<br/>角色/场景/道具库"]
LoadAssets --> BuildPrompt["组装提示模板:<br/>分镜计划/摄影/表演/细节"]
BuildPrompt --> RunPhases["并行执行三阶段:<br/>计划/摄影/表演/细节"]
RunPhases --> MergeResults["合并结果:<br/>摄影规则+表演指导+最终面板"]
MergeResults --> Persist["持久化面板:<br/>写入数据库"]
Persist --> End(["结束"])
```

**图表来源**
- [src/lib/workers/handlers/script-to-storyboard.ts](file://src/lib/workers/handlers/script-to-storyboard.ts)
- [lib/prompts/novel-promotion/agent_storyboard_plan.en.txt](file://lib/prompts/novel-promotion/agent_storyboard_plan.en.txt)
- [lib/prompts/novel-promotion/agent_storyboard_detail.en.txt](file://lib/prompts/novel-promotion/agent_storyboard_detail.en.txt)
- [lib/prompts/novel-promotion/ai_story_expand.en.txt](file://lib/prompts/novel-promotion/ai_story_expand.en.txt)
- [src/lib/workers/text.worker.ts](file://src/lib/workers/text.worker.ts)

**章节来源**
- [src/lib/workers/text.worker.ts](file://src/lib/workers/text.worker.ts)
- [src/lib/workers/handlers/script-to-storyboard.ts](file://src/lib/workers/handlers/script-to-storyboard.ts)
- [lib/prompts/novel-promotion/agent_storyboard_plan.en.txt](file://lib/prompts/novel-promotion/agent_storyboard_plan.en.txt)
- [lib/prompts/novel-promotion/agent_storyboard_detail.en.txt](file://lib/prompts/novel-promotion/agent_storyboard_detail.en.txt)
- [lib/prompts/novel-promotion/ai_story_expand.en.txt](file://lib/prompts/novel-promotion/ai_story_expand.en.txt)

### 图像生成与资产处理
- 资产类型
  - 角色：支持外观描述、多形象切换与选择索引。
  - 场景：支持多图与可用位置槽位，用于镜头布置锚点。
  - 道具：作为场景内可选元素参与分镜，支持AI设计和修改。
- Worker职责
  - 角色/场景/面板图像生成与面板变体生成。
  - 资产修改与图库批量处理。
  - **新增** 道具资产的AI设计和描述修改。
- 并发控制
  - 基于用户维度的并发门控，防止资源争用导致超卖或延迟放大。

```mermaid
sequenceDiagram
participant W as "图像Worker"
participant AH_W as "资产Hub Worker"
participant DB as "数据库"
participant M as "模型网关"
W->>DB : 读取角色/场景/面板元数据
AH_W->>M : 调用AI模型(道具设计)
M-->>AH_W : 返回道具设计结果
AH_W->>DB : 更新道具资产描述
W->>M : 请求图像生成(角色/场景/面板/变体)
M-->>W : 返回图像URL/Base64
W->>DB : 更新面板/资产图像引用
```

**图表来源**
- [src/lib/workers/image.worker.ts](file://src/lib/workers/image.worker.ts)
- [src/lib/workers/handlers/asset-hub-ai-design.ts](file://src/lib/workers/handlers/asset-hub-ai-design.ts)
- [src/lib/model-gateway/index.ts](file://src/lib/model-gateway/index.ts)
- [prisma/schema.prisma](file://prisma/schema.prisma)

**章节来源**
- [src/lib/workers/image.worker.ts](file://src/lib/workers/image.worker.ts)
- [src/lib/workers/handlers/asset-hub-ai-design.ts](file://src/lib/workers/handlers/asset-hub-ai-design.ts)
- [prisma/schema.prisma](file://prisma/schema.prisma)

### 视频合成与音频处理
- 单面板视频生成
  - 输入：面板图像URL（支持签名直链）、视频模型键、生成选项（如画幅、是否生音轨、首尾帧等）。
  - 流程：解析生成选项、构造请求、调用模型网关、下载/上传、持久化cosKey与生成模式。
- 首尾帧模式
  - 通过首帧与指定尾帧生成过渡，需目标模型支持首尾帧能力。
- 唇同步
  - 以视频与对应语音片段为输入，生成带口型同步的视频并持久化。
- 模型路由与兼容
  - 对Gemini兼容视频与官方视频请求进行路由，确保正确调用提供方生成器。

```mermaid
sequenceDiagram
participant W as "视频Worker"
participant DB as "数据库"
participant M as "模型网关"
participant COS as "对象存储"
W->>DB : 读取面板与项目配置
W->>M : 生成视频(图像/提示/选项)
M-->>W : 返回视频URL/下载头
W->>COS : 上传视频并获取cosKey
W->>DB : 写入videoUrl与生成模式
W->>M : 可选 : 唇同步(视频+音频)
M-->>W : 返回唇同步视频
W->>COS : 上传并写入lipSyncVideoUrl
```

**图表来源**
- [src/lib/workers/video.worker.ts](file://src/lib/workers/video.worker.ts)
- [src/lib/model-gateway/index.ts](file://src/lib/model-gateway/index.ts)
- [src/lib/generators/vidu.ts](file://src/lib/generators/vidu.ts)
- [tests/unit/generator-api.test.ts](file://tests/unit/generator-api.test.ts)

**章节来源**
- [src/lib/workers/video.worker.ts](file://src/lib/workers/video.worker.ts)
- [src/lib/generators/vidu.ts](file://src/lib/generators/vidu.ts)
- [tests/unit/generator-api.test.ts](file://tests/unit/generator-api.test.ts)

### 语音生成与设计
- 语音行生成
  - 根据剧集与语音行ID，调用语音生成器返回音频并持久化。
- 语音设计
  - 支持角色声音风格设计与批量生成。

```mermaid
sequenceDiagram
participant W as "语音Worker"
participant DB as "数据库"
participant TTS as "语音生成器"
W->>DB : 读取语音行与剧集
W->>TTS : 生成语音(模型/文本/速率)
TTS-->>W : 返回音频URL
W->>DB : 写入audioUrl与时长
```

**图表来源**
- [src/lib/workers/voice.worker.ts](file://src/lib/workers/voice.worker.ts)
- [prisma/schema.prisma](file://prisma/schema.prisma)

**章节来源**
- [src/lib/workers/voice.worker.ts](file://src/lib/workers/voice.worker.ts)
- [prisma/schema.prisma](file://prisma/schema.prisma)

## 智能章节建议功能

### 功能概述
智能章节建议功能通过检测用户输入的文本长度，在达到特定阈值时自动弹出智能分集建议，帮助用户将长文本自动分割为多个剧集，提升生成质量和效率。

### 触发机制
- **阈值设置**：从原来的1000字符降低到150字符
- **检测逻辑**：当用户点击"开始创作"时，系统检测文本长度
- **提示弹窗**：超过阈值时弹出智能分集建议提示

```mermaid
flowchart TD
Start(["用户点击开始创作"]) --> CheckLength["检测文本长度"]
CheckLength --> Threshold{"是否超过150字符?"}
Threshold --> |是| ShowPrompt["显示智能分集提示"]
Threshold --> |否| Continue["继续单集创作"]
ShowPrompt --> UserChoice{"用户选择"}
UserChoice --> |智能分集| SmartSplit["进入智能分集流程"]
UserChoice --> |继续创作| SingleEpisode["单集模式"]
SmartSplit --> End(["开始智能分集"])
SingleEpisode --> End2(["开始单集创作"])
Continue --> End3(["开始单集创作"])
```

**图表来源**
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/ConfigStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/ConfigStage.tsx)
- [src/components/story-input/LongTextDetectionPrompt.tsx](file://src/components/story-input/LongTextDetectionPrompt.tsx)

### 长文本检测提示组件
- **组件功能**：提供智能分集建议的弹窗提示
- **提示内容**：包含标题、描述、强烈推荐语、按钮标签等
- **交互方式**：支持智能分集和继续创作两种选择
- **国际化支持**：完整的多语言文案支持

### 智能分集流程
- **标记检测**：优先检测文本中的章节标记（如"第X集"、"Chapter X"等）
- **AI自动分集**：当未检测到标记时，使用AI模型进行智能分集
- **预览确认**：显示分集结果供用户确认和调整
- **批量保存**：支持批量保存多个剧集到数据库

```mermaid
sequenceDiagram
participant UI as "用户界面"
participant Wizard as "智能分集向导"
participant Marker as "标记检测器"
participant AI as "AI分集模型"
participant DB as "数据库"
UI->>Wizard : 上传或输入文本
Wizard->>Marker : 检测章节标记
Marker-->>Wizard : 返回检测结果
alt 检测到标记
Wizard->>AI : 跳过AI分集
AI-->>Wizard : 直接使用标记分割
else 未检测到标记
Wizard->>AI : 调用AI进行智能分集
AI-->>Wizard : 返回AI分集结果
end
Wizard->>UI : 显示分集预览
UI->>DB : 确认后批量保存
DB-->>UI : 保存成功
```

**图表来源**
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/SmartImportWizard.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/SmartImportWizard.tsx)
- [src/lib/episode-marker-detector.ts](file://src/lib/episode-marker-detector.ts)
- [src/lib/workers/handlers/episode-split.ts](file://src/lib/workers/handlers/episode-split.ts)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/smart-import/hooks/useWizardState.ts](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/smart-import/hooks/useWizardState.ts)

### 标记检测器
- **检测模式**：支持中文"第X集"、英文"Chapter X"、数字编号等多种格式
- **置信度评估**：根据匹配数量和分布计算置信度等级
- **预览分割**：生成预览分割结果供用户确认
- **自动补全**：自动补充缺失的集数内容

### AI分集模型
- **提示模板**：提供详细的分集规则和输出格式要求
- **字数均衡**：确保各集字数尽可能均衡，偏差不超过±20%
- **自然断点**：优先识别章节标记、场景编号、时间跳跃等自然断点
- **JSON输出**：严格按照JSON格式输出，包含分析、剧集列表和验证信息

**章节来源**
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/ConfigStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/ConfigStage.tsx)
- [src/components/story-input/LongTextDetectionPrompt.tsx](file://src/components/story-input/LongTextDetectionPrompt.tsx)
- [src/lib/episode-marker-detector.ts](file://src/lib/episode-marker-detector.ts)
- [src/lib/workers/handlers/episode-split.ts](file://src/lib/workers/handlers/episode-split.ts)
- [lib/prompts/novel-promotion/episode_split.en.txt](file://lib/prompts/novel-promotion/episode_split.en.txt)
- [lib/prompts/novel-promotion/episode_split.zh.txt](file://lib/prompts/novel-promotion/episode_split.zh.txt)
- [messages/zh/novel-promotion.json](file://messages/zh/novel-promotion.json)
- [messages/zh/smartImport.json](file://messages/zh/smartImport.json)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/SmartImportWizard.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/SmartImportWizard.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/smart-import/hooks/useWizardState.ts](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/smart-import/hooks/useWizardState.ts)

## AI设计道具功能

### 功能概述
AI设计道具功能通过LLM任务自动生成道具的视觉描述，支持重复检测和SHA-1哈希机制，确保相同指令不会重复执行。

### 核心流程
1. **用户提交**：用户通过API提交道具设计指令
2. **去重检测**：使用SHA-1哈希生成去重密钥
3. **任务提交**：通过LLM观察系统提交异步任务
4. **模型调用**：AI模型生成道具描述
5. **结果返回**：返回JSON格式的道具描述

```mermaid
flowchart TD
Start(["用户提交道具设计指令"]) --> Hash["SHA-1哈希计算<br/>用户ID:prop:指令"]
Hash --> Dedupe["去重检查<br/>asset_hub_ai_design_prop:{digest}"]
Dedupe --> Submit["提交LLM任务<br/>ASSET_HUB_AI_DESIGN_PROP"]
Submit --> Model["AI模型推理<br/>prop_create提示模板"]
Model --> Parse["JSON响应解析"]
Parse --> Result["返回道具描述"]
Result --> End(["完成"])
```

**图表来源**
- [src/app/api/asset-hub/ai-design-prop/route.ts](file://src/app/api/asset-hub/ai-design-prop/route.ts)
- [src/lib/workers/handlers/asset-hub-ai-design.ts](file://src/lib/workers/handlers/asset-hub-ai-design.ts)
- [src/lib/asset-utils/ai-design.ts](file://src/lib/asset-utils/ai-design.ts)

### 重复检测机制
- **哈希算法**：使用SHA-1算法对"用户ID:prop:指令"进行哈希
- **截断处理**：取哈希值的前16个字符作为去重密钥
- **存储格式**：`asset_hub_ai_design_prop:{dedupeDigest}`
- **去重效果**：相同用户对相同指令的重复请求会被识别为重复任务

### 任务处理流程
- **API层**：接收用户指令，生成去重密钥，提交LLM任务
- **Worker层**：处理资产Hub AI设计任务，调用AI模型
- **资产工具**：统一的AI设计接口，支持多种资产类型
- **提示模板**：专用的道具创建提示模板，确保输出格式

### 前端集成
- **表单组件**：支持AI设计和参考图描述提取
- **状态管理**：跟踪AI设计、描述提取和提交状态
- **错误处理**：统一的错误提示和重试机制
- **自动填充**：参考图模式下自动提取描述

**章节来源**
- [src/app/api/asset-hub/ai-design-prop/route.ts](file://src/app/api/asset-hub/ai-design-prop/route.ts)
- [src/lib/workers/handlers/asset-hub-ai-design.ts](file://src/lib/workers/handlers/asset-hub-ai-design.ts)
- [src/lib/asset-utils/ai-design.ts](file://src/lib/asset-utils/ai-design.ts)
- [src/components/shared/assets/prop-creation/hooks/usePropCreationSubmit.ts](file://src/components/shared/assets/prop-creation/hooks/usePropCreationSubmit.ts)
- [lib/prompts/novel-promotion/prop_create.en.txt](file://lib/prompts/novel-promotion/prop_create.en.txt)
- [lib/prompts/novel-promotion/prop_create.zh.txt](file://lib/prompts/novel-promotion/prop_create.zh.txt)

## 工作区UI重构

### 设计概述
工作区UI重构引入了现代化的设计理念，通过五种不同的项目布局变体和增强的编辑体验，为用户提供更加直观和高效的创作环境。

### 五种布局变体

#### 变体1：网格卡片 (v1)
标准5列网格布局，模仿系统真实卡片风格，提供完整的项目信息展示。

```mermaid
graph TB
Grid["网格卡片布局<br/>5列网格"] --> Header["标题区域<br/>项目名称"]
Grid --> Description["描述区域<br/>项目简介"]
Grid --> Stats["统计区域<br/>章节/图片/视频数量"]
Grid --> Time["时间区域<br/>最后更新时间"]
```

**图表来源**
- [src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx](file://src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx)

#### 变体2：横向滚动 (v2)
一排横滚大卡片，提供更强的沉浸感和流畅的浏览体验。

#### 变体3：紧凑列表 (v3)
左右布局的一行式列表，信息分布紧凑，适合快速浏览和筛选。

#### 变体4：突出首项 (v4)
第一个项目大卡片占满左侧，右侧两列小卡片网格，突出重要项目。

#### 变体5：极简圆点列表 (v5)
和输入框同宽的极简列表，仅显示项目名和关键数据，追求极致简洁。

**章节来源**
- [src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx](file://src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx)
- [messages/zh/workspaceRedesign.json](file://messages/zh/workspaceRedesign.json)

### 分段控制组件

#### 组件特性
分段控制组件提供iOS风格的滑动胶囊指示器，支持紧凑和填充两种布局模式：

- **紧凑模式** (`compact`)：内联块状显示，最大宽度受限
- **填充模式** (`fill`)：块级显示，填满容器宽度
- **滑动指示器**：胶囊形状的指示器，跟随按钮移动
- **统一样式**：所有标签使用相同的圆角和阴影效果

#### 使用场景
- **项目布局切换**：在五种布局变体之间快速切换
- **设置面板**：在不同的配置选项间切换
- **状态管理**：在不同的工作模式间切换

**章节来源**
- [src/components/ui/SegmentedControl.tsx](file://src/components/ui/SegmentedControl.tsx)

### 工作区重新设计页面

#### 页面结构
工作区重新设计页面提供了完整的布局演示和交互功能：

- **布局切换器**：顶部固定位置的切换器，显示当前布局名称和描述
- **布局预览**：中央区域展示当前选中的布局变体
- **翻页控制**：底部固定的翻页按钮，支持键盘快捷键
- **响应式设计**：适配不同屏幕尺寸和设备类型

#### 国际化支持
页面完全支持国际化，包括：
- 布局名称和描述的本地化
- 时间格式化的本地化处理
- 按钮文本和提示信息的本地化

**章节来源**
- [src/app/[locale]/dev/workspace-redesign/page.tsx](file://src/app/[locale]/dev/workspace-redesign/page.tsx)
- [messages/zh/workspaceRedesign.json](file://messages/zh/workspaceRedesign.json)

### 小说推广工作区集成

#### 工作区架构
小说推广工作区集成了新的UI设计，提供现代化的创作体验：

- **浮动导航**：替代传统的侧边栏导航
- **运行控制台**：最小化的运行状态显示
- **资产库集成**：无缝集成的资产管理和预览
- **多阶段工作流**：支持文本到脚本、脚本到分镜等多个阶段
- **智能章节建议**：集成智能章节建议功能，提升用户体验

#### 用户体验改进
- **简化界面**：减少视觉噪音，突出核心内容
- **增强交互**：提供更直观的操作反馈
- **响应式布局**：适配不同设备和屏幕尺寸
- **流畅动画**：使用CSS动画增强用户体验

**章节来源**
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/NovelPromotionWorkspace.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/NovelPromotionWorkspace.tsx)

## 依赖关系分析
- Worker启动与生命周期
  - 启动四个Worker实例，监听各自队列，注册ready/error/failed事件，统一记录日志与关闭流程。
  - **新增** 资产Hub Worker专门处理AI设计任务。
- 任务类型与AI任务判定
  - 所有任务类型集合均视为AI任务，便于统一计费与资源统计。
  - **新增** ASSET_HUB_AI_DESIGN_PROP任务类型。
- 并发与限流
  - 用户级并发门控结合全局并发配置，避免过载与抖动。
- 数据模型耦合
  - 小说推广项目模型与剪辑、分镜、镜头、角色、场景、道具等实体强关联，形成闭环的数据流转。
- **新增** 资产Hub任务路由
  - 通过LLM观察系统统一处理资产Hub任务，支持去重和进度报告。
- **新增** UI组件依赖
  - 分段控制组件被广泛应用于各种设置和配置界面
  - 项目布局组件为工作区提供了灵活的内容展示方案
  - 智能章节建议功能集成到主要工作流中

```mermaid
graph LR
IDX["workers/index.ts"] --> TXT["text.worker.ts"]
IDX --> IMG["image.worker.ts"]
IDX --> VID["video.worker.ts"]
IDX --> VOC["voice.worker.ts"]
IDX --> AH["asset-hub-ai-design.ts"]
WF["run-runtime/workflow.ts"] --> TXT
WF --> IMG
WF --> VID
WF --> VOC
WF --> AH
CON["workflow-concurrency.ts"] --> IMG
CON --> VID
CON --> TXT
TASK["task/types.ts"] --> AH
RT["llm-observe/route-task.ts"] --> AH
SEG["SegmentedControl.tsx"] --> UI["工作区UI组件"]
GRID["ProjectLayouts.tsx"] --> UI
RED["workspace-redesign/page.tsx"] --> UI
SMART["智能章节建议"] --> UI
```

**图表来源**
- [src/lib/workers/index.ts](file://src/lib/workers/index.ts)
- [src/lib/run-runtime/workflow.ts](file://src/lib/run-runtime/workflow.ts)
- [src/lib/workflow-concurrency.ts](file://src/lib/workflow-concurrency.ts)
- [src/lib/task/types.ts](file://src/lib/task/types.ts)
- [src/lib/llm-observe/route-task.ts](file://src/lib/llm-observe/route-task.ts)
- [src/components/ui/SegmentedControl.tsx](file://src/components/ui/SegmentedControl.tsx)
- [src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx](file://src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx)

**章节来源**
- [src/lib/workers/index.ts](file://src/lib/workers/index.ts)
- [src/lib/run-runtime/workflow.ts](file://src/lib/run-runtime/workflow.ts)
- [src/lib/workflow-concurrency.ts](file://src/lib/workflow-concurrency.ts)
- [src/lib/task/types.ts](file://src/lib/task/types.ts)
- [src/lib/llm-observe/route-task.ts](file://src/lib/llm-observe/route-task.ts)
- [src/components/ui/SegmentedControl.tsx](file://src/components/ui/SegmentedControl.tsx)
- [src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx](file://src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx)

## 性能考量
- 并发配置
  - 分析/图像/视频三类任务可独立设置并发上限，建议根据模型吞吐与硬件资源动态调整。
  - 对非法值进行归一化处理，保证稳定性。
  - **新增** 资产Hub任务的独立并发控制。
- 流式回调与进度上报
  - 文本Worker对推理过程进行分片上报，降低端到端等待时间，提升可观测性。
  - **新增** 资产Hub任务的阶段化进度报告。
- 任务拆分与并行
  - 分镜生成阶段采用多阶段并行，缩短关键路径。
  - **新增** 道具设计任务的异步处理，避免阻塞主线程。
  - **新增** 智能章节建议的异步检测，不影响主流程。
- 存储与网络
  - 视频/图像生成后统一上传对象存储，减少中间态体积与跨域风险。
  - **新增** 道具设计结果的缓存机制。
- 模型路由优化
  - 明确官方与兼容通道的路由策略，避免重复适配成本。
- **新增** 去重机制优化
  - SHA-1哈希计算开销较小，但能显著减少重复任务执行。
  - 去重密钥格式标准化，便于维护和调试。
- **新增** UI性能优化
  - 分段控制组件使用CSS动画，避免JavaScript动画的性能开销
  - 项目布局组件采用虚拟滚动技术，优化大量项目的渲染性能
  - 工作区重新设计页面使用React.memo优化重渲染
  - 智能章节建议采用防抖机制，避免频繁触发

**章节来源**
- [src/lib/workflow-concurrency.ts](file://src/lib/workflow-concurrency.ts)
- [src/lib/workers/text.worker.ts](file://src/lib/workers/text.worker.ts)
- [src/lib/workers/video.worker.ts](file://src/lib/workers/video.worker.ts)
- [src/lib/workers/handlers/asset-hub-ai-design.ts](file://src/lib/workers/handlers/asset-hub-ai-design.ts)
- [src/components/ui/SegmentedControl.tsx](file://src/components/ui/SegmentedControl.tsx)
- [src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx](file://src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx)

## 故障排查指南
- 常见错误与定位
  - 缺少必需模型键：如视频/语音/分析模型未配置，任务将直接报错。
  - 面板缺失图像/视频提示：生成视频前需确保面板存在有效图像与提示词。
  - 首尾帧模型不支持：若目标模型不支持首尾帧能力，将抛出不支持错误。
  - 语音行缺失：唇同步需要有效的语音行与音频URL。
  - **新增** 道具设计失败：检查用户指令格式、模型配置和去重机制。
  - **新增** 布局切换异常：检查CSS样式和JavaScript事件绑定。
  - **新增** 分段控制失效：验证组件属性和状态管理。
  - **新增** 智能章节建议不触发：检查阈值设置和文本检测逻辑。
- 日志与事件
  - Worker统一记录ready/error/failed事件，失败任务包含任务ID、类型、错误信息，便于快速定位。
  - **新增** 资产Hub任务的详细进度日志。
  - **新增** UI组件的调试信息和性能指标。
  - **新增** 智能章节建议的触发日志。
- 重试与补偿
  - 建议在上层API或任务运行时增加幂等与重试策略，结合任务状态机进行补偿。
  - **新增** 道具设计任务的去重处理，避免重复执行。
  - **新增** UI组件的错误边界和降级处理。
  - **新增** 智能章节建议的防抖处理，避免重复触发。
- 参数校验
  - 视频生成器对选项进行严格校验（如generateAudio类型、audioType取值），不符合将抛出明确错误。
  - **新增** 道具设计指令的格式验证和长度限制。
  - **新增** 布局变体的参数验证和回退机制。
  - **新增** 智能章节建议阈值的验证和调整。

**章节来源**
- [src/lib/workers/text.worker.ts](file://src/lib/workers/text.worker.ts)
- [src/lib/workers/image.worker.ts](file://src/lib/workers/image.worker.ts)
- [src/lib/workers/video.worker.ts](file://src/lib/workers/video.worker.ts)
- [src/lib/workers/voice.worker.ts](file://src/lib/workers/voice.worker.ts)
- [src/lib/workers/handlers/asset-hub-ai-design.ts](file://src/lib/workers/handlers/asset-hub-ai-design.ts)
- [src/lib/generators/vidu.ts](file://src/lib/generators/vidu.ts)
- [src/components/ui/SegmentedControl.tsx](file://src/components/ui/SegmentedControl.tsx)
- [src/app/[locale]/dev/workspace-redesign/page.tsx](file://src/app/[locale]/dev/workspace-redesign/page.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx)

## 结论
该工作流以任务队列为纽带，串联文本、图像、视频、语音四大阶段，配合严格的模型路由、参数校验与并发控制，实现了从小说到视频的自动化生产。通过提示模板与多阶段推理，确保画面细节与叙事节奏的一致性；通过流式回调与进度上报，显著提升用户体验与可观测性。

**更新** 新增的AI设计道具功能和工作区UI重构进一步增强了工作流的智能化水平和用户体验。AI设计道具功能通过LLM任务自动生成道具描述，结合重复检测和SHA-1哈希机制，确保了系统的高效性和稳定性。工作区UI重构引入了五种不同的项目布局变体和现代化的交互设计，为用户提供了更加直观和高效的创作环境。智能章节建议功能的改进将触发阈值从1000字符降低到150字符，使更多用户能够受益于AI驱动的章节推荐功能。

建议在生产环境中结合业务负载动态调整并发配置，并完善重试与补偿策略以提升稳定性。智能章节建议功能的阈值调整应该根据用户行为数据持续优化，以达到最佳的用户体验。

## 附录

### 关键数据模型与字段
- 小说推广项目模型（关键字段）
  - 分析/图像/视频/语音模型键
  - 画幅(videoRatio)/分辨率(videoResolution)/TTS速率(ttsRate)
  - 工作流模式(workflowMode)/艺术风格(artStyle)/风格提示(artStylePrompt)
  - 角色/场景/道具资产列表
  - 剧集/剪辑/分镜/镜头实体
- **新增** 资产Hub任务模型
  - 任务类型：ASSET_HUB_AI_DESIGN_PROP
  - 去重密钥格式：asset_hub_ai_design_prop:{16-char-digest}
  - 资产类型：prop
  - 项目ID：global-asset-hub
- **新增** 工作区UI组件模型
  - 分段控制组件：支持紧凑和填充两种布局模式
  - 项目布局组件：五种不同的布局变体
  - 响应式设计：适配不同屏幕尺寸
- **新增** 智能章节建议模型
  - 触发阈值：150字符
  - 提示文案：多语言支持
  - 分集流程：标记检测 + AI分集

**章节来源**
- [prisma/schema.prisma](file://prisma/schema.prisma)
- [prisma/schema.sqlit.prisma](file://prisma/schema.sqlit.prisma)
- [src/types/project.ts](file://src/types/project.ts)
- [src/lib/task/types.ts](file://src/lib/task/types.ts)
- [src/components/ui/SegmentedControl.tsx](file://src/components/ui/SegmentedControl.tsx)
- [src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx](file://src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx)

### AI模型选择与调用策略
- OpenAI兼容通道
  - 通过模型网关统一路由，自动识别官方与兼容提供方，保障一致性。
- Gemini兼容视频
  - 单元测试验证了Gemini兼容视频与官方视频的路由差异，确保正确调用。
- 生成器选项校验
  - 视频生成器对选项进行严格校验与默认值处理，避免无效参数导致失败。
- **新增** 资产设计模型策略
  - 统一使用分析模型进行资产设计
  - 支持用户配置覆盖和任务负载
  - 跳过计费处理，提高响应速度
- **新增** UI组件模型策略
  - 分段控制组件使用CSS动画优化性能
  - 项目布局组件采用虚拟滚动技术
  - 响应式设计使用CSS Grid和Flexbox
- **新增** 智能章节建议模型策略
  - 使用分析模型进行文本长度检测
  - 支持多语言提示文案
  - 防抖机制避免频繁触发

**章节来源**
- [src/lib/model-gateway/index.ts](file://src/lib/model-gateway/index.ts)
- [tests/unit/generator-api.test.ts](file://tests/unit/generator-api.test.ts)
- [src/lib/generators/vidu.ts](file://src/lib/generators/vidu.ts)
- [src/lib/asset-utils/ai-design.ts](file://src/lib/asset-utils/ai-design.ts)
- [src/components/ui/SegmentedControl.tsx](file://src/components/ui/SegmentedControl.tsx)

### 道具设计提示模板
- **英文模板**：要求生成50-120词的道具视觉描述，排除人物、场景等元素
- **中文模板**：要求生成60-120字的道具视觉描述，适合白底居中出图
- **输出格式**：严格JSON格式，所有引号需转义为中文引号
- **材质要求**：必须具体化材质描述，如"深棕色做旧皮革"

**章节来源**
- [lib/prompts/novel-promotion/prop_create.en.txt](file://lib/prompts/novel-promotion/prop_create.en.txt)
- [lib/prompts/novel-promotion/prop_create.zh.txt](file://lib/prompts/novel-promotion/prop_create.zh.txt)

### 任务类型对照表
- 文本任务：ANALYZE_NOVEL, STORY_TO_SCRIPT_RUN, SCRIPT_TO_STORYBOARD_RUN
- 图像任务：IMAGE_CHARACTER, IMAGE_LOCATION, IMAGE_PANEL
- 视频任务：VIDEO_PANEL, LIP_SYNC
- 语音任务：VOICE_LINE, VOICE_DESIGN
- **新增** 资产Hub任务：
  - ASSET_HUB_AI_DESIGN_CHARACTER
  - ASSET_HUB_AI_DESIGN_LOCATION  
  - ASSET_HUB_AI_DESIGN_PROP
  - ASSET_HUB_AI_MODIFY_CHARACTER
  - ASSET_HUB_AI_MODIFY_LOCATION
  - ASSET_HUB_AI_MODIFY_PROP
- **新增** UI组件任务：
  - WORKSPACE_LAYOUT_SWITCH
  - SEGMENTED_CONTROL_CHANGE
  - PROJECT_CARD_INTERACTION
- **新增** 智能章节建议任务：
  - EPISODE_SPLIT_LLM
  - EPISODE_SPLIT_MARKERS

**章节来源**
- [src/lib/task/types.ts](file://src/lib/task/types.ts)
- [tests/unit/worker/asset-hub-ai-design.test.ts](file://tests/unit/worker/asset-hub-ai-design.test.ts)
- [src/app/[locale]/dev/workspace-redesign/page.tsx](file://src/app/[locale]/dev/workspace-redesign/page.tsx)
- [src/components/ui/SegmentedControl.tsx](file://src/components/ui/SegmentedControl.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx)

### 工作区布局变体详细说明

#### 变体1：网格卡片 (v1)
- **布局特点**：标准5列网格，系统真实卡片风格
- **适用场景**：需要完整项目信息展示的场景
- **交互方式**：悬停效果、渐变背景、点击跳转
- **性能考虑**：适合中小规模项目列表

#### 变体2：横向滚动 (v2)
- **布局特点**：一行横滚大卡片，可滑动查看
- **适用场景**：需要沉浸式浏览体验的场景
- **交互方式**：触摸滑动、快节奏浏览
- **性能考虑**：使用snap滚动优化性能

#### 变体3：紧凑列表 (v3)
- **布局特点**：单行式列表，信息左右分布
- **适用场景**：需要快速筛选和比较的场景
- **交互方式**：悬停显示详情、快速导航
- **性能考虑**：适合大量项目的快速浏览

#### 变体4：突出首项 (v4)
- **布局特点**：首个项目大卡片+右侧小卡片网格
- **适用场景**：需要突出重要项目或最新项目的场景
- **交互方式**：主次分明的层次结构
- **性能考虑**：大卡片和小卡片混合布局

#### 变体5：极简列表 (v5)
- **布局特点**：和输入框同宽的极简圆点行
- **适用场景**：追求极致简洁和空间利用的场景
- **交互方式**：最小化视觉干扰
- **性能考虑**：最少的DOM节点和样式计算

**章节来源**
- [src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx](file://src/app/[locale]/dev/workspace-redesign/ProjectLayouts.tsx)
- [messages/zh/workspaceRedesign.json](file://messages/zh/workspaceRedesign.json)

### 智能章节建议详细说明

#### 触发阈值
- **阈值设置**：150字符（从原来的1000字符降低）
- **检测时机**：用户点击"开始创作"时
- **检测逻辑**：计算文本去除空白字符后的长度
- **用户体验**：更低的门槛让更多用户受益

#### 提示文案
- **标题**："🚀 建议使用智能分集"
- **描述**：显示检测到的文本字数，提醒长文本可能影响生成效果
- **强烈推荐**：详细的智能分集优势说明
- **按钮标签**：智能分集（推荐）、仍然单集创作
- **多语言支持**：完整的国际化文案

#### 分集流程
- **标记检测**：优先检测章节标记（如"第X集"、"Chapter X"等）
- **AI分集**：未检测到标记时使用AI模型进行智能分集
- **预览确认**：显示分集结果供用户确认和调整
- **批量保存**：支持批量保存多个剧集到数据库

#### 技术实现
- **组件集成**：集成到NovelInputStage组件中
- **状态管理**：使用useState和useEffect管理提示状态
- **防抖处理**：避免频繁触发导致的性能问题
- **国际化支持**：完整的多语言文案支持

**章节来源**
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/NovelInputStage.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/ConfigStage.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/ConfigStage.tsx)
- [src/components/story-input/LongTextDetectionPrompt.tsx](file://src/components/story-input/LongTextDetectionPrompt.tsx)
- [messages/zh/novel-promotion.json](file://messages/zh/novel-promotion.json)
- [src/lib/episode-marker-detector.ts](file://src/lib/episode-marker-detector.ts)
- [src/lib/workers/handlers/episode-split.ts](file://src/lib/workers/handlers/episode-split.ts)
- [lib/prompts/novel-promotion/episode_split.en.txt](file://lib/prompts/novel-promotion/episode_split.en.txt)
- [lib/prompts/novel-promotion/episode_split.zh.txt](file://lib/prompts/novel-promotion/episode_split.zh.txt)
- [messages/zh/smartImport.json](file://messages/zh/smartImport.json)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/SmartImportWizard.tsx](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/SmartImportWizard.tsx)
- [src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/smart-import/hooks/useWizardState.ts](file://src/app/[locale]/workspace/[projectId]/modes/novel-promotion/components/smart-import/hooks/useWizardState.ts)