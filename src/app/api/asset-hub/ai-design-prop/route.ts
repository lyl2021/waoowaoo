...
export const POST = apiHandler(async (request: NextRequest) => {
  ...
  const dedupeDigest = createHash('sha1')
    .update(`${session.user.id}:prop:${userInstruction}`)
    .digest('hex')
    .slice(0, 16)
  ...
  const asyncTaskResponse = await maybeSubmitLLMTask({
    ...
    type: TASK_TYPE.ASSET_HUB_AI_DESIGN_PROP,
    targetType: 'GlobalAssetHubPropDesign',
    targetId: session.user.id,
    routePath: '/api/asset-hub/ai-design-prop',
    body: payload,
    dedupeKey: `asset_hub_ai_design_prop:${dedupeDigest}`})
